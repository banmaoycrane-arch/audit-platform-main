...
- 创建账套时可指定会计时间线起点（`accounting_start_date`），作为该账套期间与报表的时间基准。
...
- 审计底稿协作包：底稿版本固化文件哈希与工作簿快照，复核关联提交/批准/合并版本，批注可定位到工作表单元格。
- 审计协作通知中心：复核与协作事件推送站内通知，支持已读管理与协作台展示。
- 多 LLM 解析引擎：可配置多个 LLM 引擎与对比策略，解析结果附带客观置信度与分阶段耗时诊断。
- 模块台账行级维护：合同、发票、银行流水、出入库等台账支持在线编辑、纠错、归档与删除。
