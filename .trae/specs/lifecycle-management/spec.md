...
会计账套、业务台账、项目三者有不同的生命周期驱动因素：
- **会计账套**：跟随客户合作周期，客户开始合作时创建，合作终止时归档
...
### Requirement: 会计账套生命周期

系统 SHALL 为会计账套定义以下生命周期：
...
#### 与会计账套的关系
- 业务台账的 `locked` 状态由会计账套的期间结账触发
- 会计账套 `archived` 时，所有关联台账自动 `locked`
...
#### 与会计账套的关系
- 项目可关联多个会计账套（跨年度审计）
- 项目 `completed` 时，关联账套不自动归档（账套生命周期独立）
- 项目 `completed` 后，仍可查看关联账套的历史数据
...
- **会计账套跟随客户周期**：客户是会计账套的生命周期驱动者。客户开始合作 → 创建账套；客户暂停 → 账套暂停；客户终止 → 账套归档。这符合事务所的实际业务节奏。
- **业务台账跟随模块周期**：银行按月、税务按季、库存按日。台账的关闭和开启由模块自身决定，不受会计账套直接影响（但会计结账会锁定台账）。
- **项目管理周期最大化**：审计项目完成后，所有工作底稿、审计证据、沟通记录必须永久保留。这是审计准则的要求（审计底稿保存期限通常为 10 年以上）。项目可以 reopen，因为后续发现重大问题需要追加审计程序。
- **为什么区分生命周期**：三者的驱动因素不同，混在一起会导致状态管理混乱。比如客户暂停合作（账套 suspended），但税务模块仍在做季度申报（台账 active），同时审计项目已完成（project completed）。三者独立管理，互不影响。
