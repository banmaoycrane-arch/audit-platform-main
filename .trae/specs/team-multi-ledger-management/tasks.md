...
- [x] Task 3: 后端账簿管理 API
  - [x] SubTask 3.1: 新增 `backend/app/services/ledger_service.py` — 创建账簿、授权用户、切换账簿、获取用户账簿列表
...
- [x] Task 4: 后端认证中间件扩展
  - [x] SubTask 4.1: 修改 `backend/app/core/dependencies.py` — `get_current_user` 返回用户 + 当前账簿
  - [x] SubTask 4.2: 新增 `get_current_ledger` 依赖 — 从请求 Header 或 Token 中获取当前账簿 ID
  - [x] SubTask 4.3: 验证用户是否有当前账簿的访问权限

- [x] Task 5: 前端账簿选择器与状态管理
  ...
  - [x] SubTask 5.3: 新增 `frontend/src/components/LedgerSelector.tsx` — 账簿选择器组件（Dropdown 或 Select）
  - [x] SubTask 5.4: 登录后自动获取用户账簿列表，如果有默认账簿自动进入

- [x] Task 6: 前端首页与工作台改造
  - [x] SubTask 6.1: 修改 `frontend/src/pages/WorkspacePage.tsx` — 顶部增加账簿选择器 + 团队名称 + 用户名称
  - [x] SubTask 6.2: 大盘数据根据当前账簿过滤显示
  - [x] SubTask 6.3: 切换账簿后自动刷新页面数据
  - [x] SubTask 6.4: 如果用户无授权账簿，显示"请联系管理员分配账簿"

- [x] Task 7: 测试与验证
  - [x] SubTask 7.1: 新增 `backend/tests/test_ledger_api.py` — 创建账簿、授权、切换、数据隔离
  - [x] SubTask 7.2: 运行后端 `pytest -q`
  - [x] SubTask 7.3: 运行前端 `npm run lint`
  - [x] SubTask 7.4: 手动验证：登录后选择账簿、切换账簿数据隔离、默认账簿自动进入
