...
当前项目功能增长较快，已经覆盖登录注册、首次引导、工作台、团队/账套/项目、凭证流程、基础资料、报表、审计、银行、税务、Agent 等多个模块。虽然大部分页面已接入 Ant Design 和统一 Shell，但不同阶段实现的页面在布局、导航、信息层级、控件密度、错误提示和占位页面风格上可能存在不一致。
