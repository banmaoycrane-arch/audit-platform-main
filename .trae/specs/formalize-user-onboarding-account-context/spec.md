# 首次登录引导与用户账簿关系正式化 Spec

## Why

当前首次登录引导并不完全是 mock：它已经调用真实的团队、账簿 API 创建数据；但它仍存在明显的临时逻辑和占位内容，例如短信验证码为开发模拟、用户协议/隐私政策为占位文本、注册后只根据团队/账簿数量简单跳转。更关键的是，用户注册后与历史账号资料、团队、账簿、项目、会计主体、已导入数据之间的关系没有被正式建模，后续容易造成数据归属不清、权限不清、项目延续困难。

本 spec 的目标是把“注册用户进入系统后到底属于哪个团队、哪个账簿、哪个项目、哪些历史数据”的关系正式定义清楚，避免系统继续依赖临时判断，降低后续开发和用户使用风险。

## What Changes

- 明确当前首次登录引导的真实与 mock 边界。
- 新增“用户上下文状态”概念，用于判断用户是否已完成团队、账簿、项目、会计主体绑定。
- 注册/登录后不再仅凭 `teams.length === 0 || ledgers.length === 0` 判断是否进入 onboarding。
- 引导流程从“创建团队 → 创建账簿”升级为“确认身份 → 选择/创建团队 → 选择/创建账簿 → 选择/创建项目 → 绑定或确认会计主体 → 进入工作台”。
- 支持用户认领或关联历史资料，例如已有账号、已有团队授权、历史账簿、已导入凭证、项目资料。
- 对未完成匹配的用户赋予临时工作状态，允许进入系统但明确提示“尚未绑定正式团队/账簿/项目”。
...
- Affected specs:
  ...
  - `team-multi-ledger-management`：团队、账簿、授权、当前账簿。
  - `ledger-register-project-concept-unification`：项目、账簿、成员关系。
...
- **WHEN** 用户登录后已经有团队、账簿、项目和当前账簿授权
- **THEN** 系统 SHALL 直接进入工作台
- **AND** 当前团队、账簿、项目状态 SHALL 可被前端读取

#### Scenario: 用户未完成上下文绑定

- **WHEN** 用户登录后缺少团队、账簿、项目或会计主体绑定
...
- **WHEN** 新用户注册且没有历史团队/账簿/项目授权
- **THEN** 系统 SHALL 引导用户创建或加入团队
- **AND** 再创建或选择账簿和项目

#### Scenario: 用户存在历史资料

- **WHEN** 用户手机号、用户名或受邀授权可以匹配历史团队/账簿/项目
...
- **WHEN** 用户已注册但尚未完成团队/账簿/项目匹配
- **THEN** 系统 MAY 允许进入受限工作台
- **AND** 用户角色 SHALL 标识为 temporary 或 onboarding_pending
- **AND** 系统 SHALL 明确提示该状态下数据只是临时工作区，需绑定正式团队/账簿/项目后才能归档为正式资料
...
- **WHEN** 系统发现已有导入任务、凭证、项目或账簿可能属于当前用户
- **THEN** 系统 SHALL 将其列为“待确认历史资料”
- **AND** 用户或管理员确认前，不 SHALL 自动并入正式账簿
...
注册/登录后 SHALL 不再仅根据团队列表或账簿列表长度决定跳转。系统 SHALL 调用用户上下文接口，由后端明确返回：是否需要 onboarding、缺失哪些绑定、推荐下一步动作。

### Requirement: 首次登录引导流程

首次登录引导 SHALL 从简单创建团队/账簿页面，升级为根据用户上下文状态动态展示的引导流程。每一步都应说明其财务含义，例如团队表示权限协作范围，账簿表示核算数据范围，项目表示具体工作承载对象，会计主体表示核算边界。
