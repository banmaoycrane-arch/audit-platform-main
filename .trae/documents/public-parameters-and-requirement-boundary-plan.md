...
- 这类问题容易被误判为登录、账簿、权限、dashboard 等业务 bug。
...
- 这会导致“同一账号登录后团队、账簿、主体配置像丢失了一样”，实质上可能是连接了不同 SQLite 文件。
...
同一项目从不同目录启动不应产生两个数据库文件。否则用户会看到“团队、账簿、会计主体配置丢失”。
...
防止未来修一个登录问题时又改导航、dashboard、账簿规则；也防止修一个端口问题时误改认证逻辑。
...
7. 本计划不处理登录业务、不处理账簿业务、不处理导航 UI、不处理报表或审计逻辑。
