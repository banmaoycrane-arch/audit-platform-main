...
| D02 | 团队、账簿、项目、上下文 | team-multi-ledger-management | 稳定 | Team/Ledger/Project 边界已冻结 |
...
| T1-5 | 创建账簿 | API `POST /api/ledger` 或账簿管理页面 | 账簿管理 |
| T1-6 | 账簿切换上下文 | 切换默认账簿，确认后续操作以 `ledger_id` 为边界 | — |
...
| 序号 | 测试项 | 验证方法 | 对应页面/路由 |
|---|---|---|---|
| T4-1 | Step1 — 账簿与期间选择 | 选账簿、选期间，检查上下文传递 | /accounting/step/1 |
...
| 序号 | 测试项 | 验证方法 | 对应页面/路由 |
|---|---|---|---|
| T5-1 | Step1 — 审计计划/任务创建 | 创建审计项目，关联账簿 | /audit/step/1 |
...
| 序号 | 测试项 | 验证方法 |
|---|---|---|
| T8-1 | Team/Ledger/Project 边界隔离 | Team 下新增账簿，不影响其他 Team 数据 |
