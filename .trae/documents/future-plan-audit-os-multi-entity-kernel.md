# 规划阶段预期方案：审计操作系统 + 多主体会计引擎

> **文档类型**: 规划阶段预期方案
> **评审日期**: 2026-07-01
> **文档状态**: ⚠️ 未来规划（非当前执行方案）
> **评审范围**: 核心数据模型设计（Team/Project/Ledger/Organization/AccountingEntry/Voucher）
> **重要声明**: 本文档记录的是项目未来演进的**目标架构愿景**和**设计方向**，不是当前 Sprint 或阶段要执行的具体开发任务。当前阶段仍按已有 MVP 路线执行，本方案用于指导后续版本的方向选择和设计决策。
...
| 变更日期 | 变更内容 | 影响范围 | 关联文档 | 状态 |
|----------|----------|----------|----------|------|
| 2026-07-01 | 首次架构评审完成，形成未来规划方案 | 全局 | 本文件 | 规划中 |
| 待定 | Ledger 类型扩展：Statutory/Tax/Management/Consolidation | Ledger 模型 | - | 未来 |
| 待定 | Organization 升级为 Legal Entity + Control Graph | Organization 模型 | - | 未来 |
| 待定 | 添加 Scope / Engagement Rule 层 | Project-Ledger 关联 | - | 未来 |
| 待定 | 语义层（Semantic Layer）设计 | 新增模块 | - | 未来 |
