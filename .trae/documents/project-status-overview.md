...
| 税务出口池 | 有壳 | 未接真实税局 |
| **经济事件工单** | **E1–E4 完成** | **L5；E3 Agent + E4 向量 2026-08-02 落地；生产升 0034 待运维** |
...
| **P0** | 生产 Alembic 收口：stamp 0028 → 0034 + staging 复验 — **需运维**（脚本：`deploy/upgrade_prod_alembic_to_0034.sh`，清单：`deploy/ALEMBIC_0028_TO_0034.md`） | 部署前必须 |
| **P0** | L6 人工签字（路径 A + 路径 B）→ 解冻 API 收敛 Phase 2 — **需会计专业用户** | M0 收尾 |
| **P1** | 生产/演示样例账：`python scripts/seed_demo_ledger.py`（可选指向生产 BASE_URL） | 填充分录与事件空洞 |
