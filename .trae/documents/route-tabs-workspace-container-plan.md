...
顶部主导航/系统栏
左侧功能模块导航
内容区顶部：已打开页面标签页，例如：首页 / 账簿管理 / 批量打印 / 移交申请
内容区主体：当前标签对应的页面容器
```
...
6. 账簿切换时清理账簿相关业务标签，避免跨账簿误操作。
7. 增加主要业务路由标题映射，让标签显示“账簿管理”“支持性文件”“审计工作底稿”等业务名称。
...
1. 财务系统强依赖当前账簿、当前项目、当前用户权限。浏览器新标签页过多时，容易出现多个标签页上下文不一致。
2. 系统内页签可以统一处理账簿切换、权限守卫和标签清理。
...
const ROUTE_TITLE_MAP = [
  ...
  { pattern: /^\/ledger-management$/, title: '账簿管理' },
...
- 标签本身不缓存页面组件，因此账簿切换后当前页面会按新账簿重新加载。
- 但已打开标签的业务含义可能来自旧账簿。
- 在 `RouteTabs` 中监听 `currentLedgerId`，当账簿切换时：
  - 保留 `/workspace`；
  - 保留 `/team-management`、`/ledger-management`、`/projects` 等管理页；
  - 关闭账簿数据页面标签，例如凭证、账簿、支持性文件、台账、审计底稿。

为什么：

- 财务系统最忌讳跨账簿误操作。
- 即使不做 keep-alive，也应避免用户误以为某个标签仍属于原账簿。
...
- 真正 keep-alive 会涉及账簿安全、权限变化、表单未保存、内存占用等风险。
...
- 凭证、账簿、报表、台账、审计底稿都必须有明确账簿归属。
- 如果切换账簿后还保留旧账簿标签，用户可能误以为当前页面仍属于旧账簿，产生跨账簿误操作风险。
