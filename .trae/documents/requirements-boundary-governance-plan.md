...
问题：早期规格仍使用 `organization_id`，后续规格要求正式核算数据以 `ledger_id` 为边界。会计主体、项目、团队、账套之间的口径需要冻结，否则报表、期初、凭证、审计测试都会重复改上下文。
...
该规格同时包含账套文件管理、客户上下文识别、行业科目模板，建议拆成三个独立域。
...
| Ledger | 会计账套，正式核算数据边界 |
| Project | 审计/记账/税务等工作项目 |
| Register | 业务台账，归属于业务模块，不等于会计账套 |
