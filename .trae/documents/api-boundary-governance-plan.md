...
> **创建日期**: 2026-07-04  
> **更新日期**: 2026-07-05  
> **代码真值**: [code-truth-status.md](./code-truth-status.md) @ `99a15db`  
...
| **Phase 2** | 前端与测试迁移到 `/api/vouchers`；废弃 `/entries/vouchers/*` | P0 | ✅ 2026-08-02 完成：6 端点 deprecated 四重注明（OpenAPI deprecated=True + docstring .. deprecated:: + tags deprecated:entries-vouchers + Deprecation/Sunset/Link 响应头指向 /api/vouchers）；client.ts 6 方法 JSDoc @deprecated；纯 entries 子资源 API 保留；**不做 307 重定向因复合键非双射**。存量 3 处调用未强制迁移（功能不等价），新建功能必须使用 /api/vouchers。S2-4 client.ts 去除 Engine 暗示：parseImportedSourceFile / getParsingRuntimeStatus / getDocumentParsingConfig / saveDocumentParsingConfig |
...
| **Phase 4** | Tag 服务统一设计 spec | P1 | ✅ 2026-08-01 完成（spec 三件套 `.trae/specs/tag-unification/` + 实施落地：`/api/entries/{id}/tags` GET/POST/DELETE 三端点改 307 重定向到 `/api/entry-tags`，保留 query；OpenAPI `deprecated=True` + docstring `.. deprecated::` + tags `deprecated:entries-tags`；`PATCH /{id}/tags` 暂保留（前序 spec 决定）；DocumentTag 向量 `search_similar_tags` / `sync_pending_tags` 预留 `ledger_id` 参数位（值 None，模型字段未加）；前端 `client.ts` 早前已迁移无残留；894/894 测试通过含 4 新 redirect 测试 + 4 新 ledger_id 回归测试；DocumentTag 实串库（模型加 `ledger_id` + Alembic 迁移）后置到生产 Alembic 0028→0034 收口后） |
