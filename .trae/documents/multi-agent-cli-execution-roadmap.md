...
- Agent 权限需要绑定用户、团队、账套、项目和业务角色，并通过多 Agent 分工适配真实人员管理。
...
- 账套模型：`backend/app/models/ledger.py`
- JWT 鉴权：`backend/app/core/dependencies.py`
- 账套上下文：`X-Ledger-Id` 或 `current_user.last_ledger_id`
- 账套权限校验：`ledger_management_service.user_has_ledger_access(...)`

重要风险与调整方向：

- `backend/app/api/routes_agent.py` 当前没有强制后端鉴权依赖。
- 后续必须把 Agent 接口级鉴权作为前置依赖先补齐。
- 鉴权逻辑不应因为“导航型 Agent”或“执行型 Agent”而分裂；页面导航、任务规划、工具执行都应复用同一套 `get_current_user`、账套权限和角色权限逻辑。
...
共同执行标准：
后端接口级鉴权 → 用户身份 → 执行来源识别（人工/CLI/Agent）→ 账套权限 → Agent角色/人员角色 → 业务规则 → 数据校验 → 风险分级 → 人工确认 → 全路径审计留痕
```
...
- Agent 后期会接触财务数据，必须从第一步就绑定真实登录用户和账套。
...
目标：明确不同 Agent 能做什么、不能做什么，并与用户权限、账套权限、人员管理、业务风险等级绑定。
...
| `admin_agent` | 系统管理员助理 | 查询团队、账套、用户配置 | 越权访问其他账套 |
...
- Agent 不能任意执行后端代码。
- 白名单工具类似“岗位授权清单”。
- 便于审计追踪：谁让哪个 Agent 在哪个账套执行了哪个工具。
...
- CLI 命令操作：CLI 服务提交命令时必须携带用户身份、来源和账套上下文。
...
- Agent 自动执行后，必须知道“谁发起、在哪个账套、执行了什么、结果是什么”。
...
因此，即使更换更强的模型，也不能绕过鉴权、账套权限、工具白名单和人工确认。
...
| `permission_agent` | 判断用户、账套、项目、工具权限 | 高 |
...
2. 复用同一套 `get_current_user`、账套权限和角色权限逻辑，不为 Agent 单独开匿名或特殊权限通道。
...
7. Agent 权限必须绑定用户、账套、团队和项目上下文，并服务于真实人员管理。
8. 高风险财务动作必须人工确认。
