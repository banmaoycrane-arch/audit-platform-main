...
def test_revise_rejects_source_file_from_other_ledger(client):
    headers_a = _auth_headers(client, "wp_user4a")
    ledger_headers_a, ledger_id_a = _create_ledger(client, headers_a)
    _, file2_id = _seed_archived_source_files(ledger_id_a)

    headers_b = _auth_headers(client, "wp_user4b")
    ledger_headers_b, ledger_id_b = _create_ledger(client, headers_b)
    _seed_archived_source_files(ledger_id_b)
    ...
    assert "does not belong to ledger" in response.json()["detail"]
