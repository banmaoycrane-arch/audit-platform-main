...
from app.services.risk_analysis_service import explain_risk
from app.services.vector_store_service import safe_vector_store


def _amount(entry: AccountingEntry) -> float:
    return float(entry.debit_amount or entry.credit_amount or 0)


def _find_similar_entries(db: Session, entry: AccountingEntry, threshold: float = 0.85) -> list[tuple[str, float, dict]]:
...
def generate_vector_similarity_risks(db: Session, import_job_id: int, seen: set) -> list[AuditRisk]:
    """基于向量相似检索生成风险"""
...
def generate_risks(db: Session, import_job_id: int) -> list[AuditRisk]:
    ...
    duplicate_groups: dict[tuple[str, float], list[AccountingEntry]] = defaultdict(list)
    for entry in entries:
