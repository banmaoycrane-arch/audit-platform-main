...
class EntityManagementService:
    ...
    def add_entity_tag(self, entity_id: int, tag: str, tag_type: str = "name", confidence: float = 0.8) -> None:
        """添加主体语义标签"""
    ...
    def add_entity_to_virtual_set(self, set_id: int, entity_id: int, member_role: Optional[str] = None) -> None:
        """将主体添加到虚拟集合"""
    ...
    def initialize_default_relation_types(self) -> None:
        """初始化默认关系类型"""
        default_types = [
    ...
    def add_entity_to_scope(self, scope_id: int, entity_id: int, member_type: str = "full",
                           ownership_percentage: Optional[float] = None) -> None:
        """将主体添加到范围"""
