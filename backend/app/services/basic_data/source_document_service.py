...
class InvoiceData(BaseModel):
    ...
    items: list[dict[str, Any]] = []  # 商品明细
    total_amount: Optional[str] = None  # 价税合计金额
    tax_amount: Optional[str] = None  # 税额
    tax_rate: Optional[str] = None  # 税率
...
class InventoryReceipt(BaseModel):
    ...
    items: list[dict[str, Any]] = []  # 商品明细
    total_amount: Optional[str] = None  # 总金额
...
class SourceDocumentResult(BaseModel):
    ...
    data: dict[str, Any]  # 具体解析结果（对应类型的模型数据）
...
def _extract_amount(text: str, pattern: str) -> Optional[str]:
    """
    使用正则表达式从文本中提取金额
    ...
    if match:
        ...
        try:
            decimal_amount = Decimal(amount_str).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
...
def parse_invoice(path: str, file_name: str = "") -> SourceDocumentResult:
    ...
    # 优先匹配“开票日期”上下文，再回退到全局日期搜索
    date_match = re.search(
        r"开票日期[：:]\s*(\d{4}-\d{2}-\d{2}|\d{4}年\d{1,2}月\d{1,2}日|\d{4}/\d{2}/\d{2})",
        raw_text,
    )
    ...
    if tax_rate_match:
        ...
        if "%" in tax_rate_match.group(0):
            rate_decimal = (rate_decimal / Decimal("100")).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)
        tax_rate_str = str(rate_decimal)

    # 提取金额
    total_amount_str = None
    total_match = re.search(r"价税合计[（(]大写[）)][^\d]*([\d,]+(?:\.\d{2})?)", raw_text)
    if total_match:
        total_amount_str = str(Decimal(total_match.group(1).replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
    ...
    if not total_amount_str:
        ...
        if amounts:
            try:
                total_amount_str = str(Decimal(amounts[-1].replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
            except ValueError:
                pass
    ...
    if tax_match:
        tax_amount_str = str(Decimal(tax_match.group(1).replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
...
def parse_bank_statement(path: str, file_name: str = "") -> SourceDocumentResult:
    ...
    for idx, row in df.iterrows():
        ...
        if amount_col:
            ...
            if pd.notna(amt_val):
                try:
                    amount_decimal = Decimal(str(amt_val).replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                except ValueError:
                    amount_decimal = Decimal("0.00")
...
def parse_contract(path: str, file_name: str = "") -> SourceDocumentResult:
    ...
    for pattern in [
        ...
        if match:
            try:
                amount = str(Decimal(match.group(1).replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP))
                break
            except ValueError:
                pass
...
def parse_inventory_receipt(path: str, file_name: str = "") -> SourceDocumentResult:
    ...
    for idx, row in df.iterrows():
        ...
        if quantity_col and pd.notna(row.get(quantity_col)):
            try:
                item["quantity"] = str(Decimal(str(row[quantity_col]).replace(",", "")).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP))
            except ValueError:
                item["quantity"] = "0.0000"
        if price_col and pd.notna(row.get(price_col)):
            try:
                item["price"] = str(Decimal(str(row[price_col]).replace(",", "")).quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP))
            except ValueError:
                item["price"] = "0.0000"
        if amount_col and pd.notna(row.get(amount_col)):
            try:
                item_amount = Decimal(str(row[amount_col]).replace(",", "")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
                item["amount"] = str(item_amount)
    ...
    inventory_receipt = InventoryReceipt(
        ...
        supplier=supplier,
        items=items,
        total_amount=str(total_amount) if total_amount > 0 else None
    )
