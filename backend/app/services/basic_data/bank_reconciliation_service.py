...
def build_draft(
    ...
    period_end: date,
    *,
    statement_balance: Decimal | str | float | int | None = None,
) -> dict[str, Any]:
    account = (
    ...
    current_balance = Decimal(str(account.current_balance)) if account.current_balance else Decimal("0.00")
    stmt_balance = Decimal(str(statement_balance)) if statement_balance is not None else current_balance
    ...
    adjusted_statement = (stmt_balance + outstanding_deposit - outstanding_payment).quantize(Decimal("0.01"))
    adjusted_book = (book_balance + bank_credit - bank_debit).quantize(Decimal("0.01"))
    difference = (adjusted_statement - adjusted_book).quantize(Decimal("0.01"))
    status = "balanced" if difference == Decimal("0") else "draft"

    reconciliation = BankReconciliation(
        ledger_id=ledger_id,
        bank_account_id=bank_account_id,
        period_end=period_end,
        statement_balance=stmt_balance.quantize(Decimal("0.01")),
        book_balance=book_balance.quantize(Decimal("0.01")),
