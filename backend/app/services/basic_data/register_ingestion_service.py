...
from app.db.models import SourceFile, ContractParty
from app.services.doc_parsing.document_parsing_service import DocumentParsingService
from app.services.agent.draft_semantic_decomposition_service import (
    ModuleTarget,
    SemanticDecomposition,
    decompose_draft,
)
from app.services.shared.ledger_service import BusinessType, ContractLedger, LedgerEntry, ledger_service
...
def _persist_contract_from_parser(
    ...
    """
    from app.services.basic_data.contract_parser_service import ContractParseResult
    ...
    for obligation in result.performance_obligations:
        performance_obligations.append({
            ...
            "quantity": str(Decimal(str(obligation.quantity)).quantize(Decimal("0.00"))) if obligation.quantity else "0.00",
            "unit": obligation.unit,
            "unit_price": str(Decimal(str(obligation.unit_price)).quantize(Decimal("0.00"))) if obligation.unit_price else "0.00",
            "total_price": str(Decimal(str(obligation.total_price)).quantize(Decimal("0.00"))) if obligation.total_price else "0.00",
            ...
            "standalone_selling_price": str(Decimal(str(obligation.standalone_selling_price)).quantize(Decimal("0.00"))) if obligation.standalone_selling_price else "0.00",
    ...
    risk_flags = {
        "penalties": [
            {
                ...
                "penalty_amount": str(Decimal(str(p.penalty_amount)).quantize(Decimal("0.00"))) if p.penalty_amount else "0.00",
                "penalty_type": p.penalty_type,
                "is_probable": p.is_probable,
                "provision_required": p.provision_required,
                "provision_amount": str(Decimal(str(p.provision_amount)).quantize(Decimal("0.00"))) if p.provision_amount else "0.00",
        ...
        "contract_costs": [
            {
                ...
                "amount": str(Decimal(str(c.amount)).quantize(Decimal("0.00"))) if c.amount else "0.00",
        ...
        "financial_assets": [
            {
                ...
                "amount": str(Decimal(str(a.amount)).quantize(Decimal("0.00"))) if a.amount else "0.00",
                "expected_credit_loss": str(Decimal(str(a.expected_credit_loss)).quantize(Decimal("0.00"))) if a.expected_credit_loss else "0.00",
        ...
        "tax_treatment": {
            ...
            "tax_amount": str(Decimal(str(result.tax_treatment.tax_amount)).quantize(Decimal("0.00"))) if result.tax_treatment.tax_amount else "0.00",
    ...
    payload = _scope_payload(
        ...
        contract_amount=Decimal(str(result.price.total_amount)).quantize(Decimal("0.00")) if result.price.total_amount else Decimal("0.00"),
        currency=result.price.currency,
        tax_rate=float(result.price.tax_rate) if result.price.tax_rate else 0,
        tax_amount=Decimal(str(result.price.tax_amount)).quantize(Decimal("0.00")) if result.price.tax_amount else Decimal("0.00"),
        performance_obligations=performance_obligations,
        transaction_price=Decimal(str(result.price.amount_excl_tax)).quantize(Decimal("0.00")) if result.price.amount_excl_tax else Decimal("0.00"),
        is_over_time=any(
    ...
    contract_parties = db.query(ContractParty).filter(ContractParty.contract_id == contract.id).all()
    base_data = {
        "contract_number": contract.contract_no,
        "party_a": next((p.party_name for p in contract_parties if p.party_role == "party_a"), ""),
        "party_b": next((p.party_name for p in contract_parties if p.party_role == "party_b"), ""),
        "sign_date": result.signing_date,
        "contract_amount": str(Decimal(str(result.price.total_amount)).quantize(Decimal("0.00"))) if result.price.total_amount else "0.00",
        "amount": str(Decimal(str(result.price.amount_excl_tax)).quantize(Decimal("0.00"))) if result.price.amount_excl_tax else "0.00",
    ...
    base_entry.metadata["contract_parse_result"] = {
        ...
        "price": {
            "total_amount": str(Decimal(str(result.price.total_amount)).quantize(Decimal("0.00"))) if result.price.total_amount else "0.00",
            "amount_excl_tax": str(Decimal(str(result.price.amount_excl_tax)).quantize(Decimal("0.00"))) if result.price.amount_excl_tax else "0.00",
            "tax_rate": float(result.price.tax_rate) if result.price.tax_rate else 0,
            "tax_amount": str(Decimal(str(result.price.tax_amount)).quantize(Decimal("0.00"))) if result.price.tax_amount else "0.00",
        },
...
def _persist_bank_statements(
    ...
    for index, transaction in enumerate(transactions, start=1):
        payload = _scope_payload(
            ...
            amount=abs(Decimal(str(transaction.get("amount") or 0)).quantize(Decimal("0.00"))),
...
def _persist_payroll(
    ...
    from app.services.shared.ledger_service import PayrollLedger
...
def ingest_register_from_document(
    ...
    try:
        ...
        return RegisterIngestionResult(
            success=False,
            document_type=document_type,
            module_label=general_def["label"],
            module_path=general_def["module_path"],
            confidence=result.confidence,
