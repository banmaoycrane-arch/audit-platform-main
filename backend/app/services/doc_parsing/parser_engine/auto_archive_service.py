...
def _as_decimal(value: Any) -> Decimal | None:
    if value is None or value == "":
        return None
    try:
        return Decimal(str(value).replace(",", "")).quantize(Decimal("0.00"))
...
def _get_data(parser_result: dict[str, Any]) -> dict[str, Any]:
    """安全提取 parser_result 中的结构化数据字典。"""
    raw_data = parser_result.get("data")
...
def _archive_bank(db: Session, job: ImportJob, source_file: SourceFile, parser_result: dict[str, Any]) -> tuple[str, int]:
    ...
    if job.ledger_id:
        ...
        db.add(BankTransaction(
            ...
            ledger_id=job.ledger_id,
            transaction_date=statement.transaction_date or datetime.now(timezone.utc).date(),
...
def _archive_voucher(db: Session, job: ImportJob, source_file: SourceFile, parser_result: dict[str, Any]) -> tuple[str, int]:
    ...
    entry = AccountingEntry(
        ...
        source_file_id=source_file.id,
        entry_source="auto",
        debit_amount=_as_decimal(_pick(data, "debit_amount", "借方金额")) or Decimal("0.00"),
        credit_amount=_as_decimal(_pick(data, "credit_amount", "贷方金额")) or Decimal("0.00"),
