...
def parse_invoice_rules(text: str, file_path: str = "") -> dict[str, Any]:
    ...
    # 价税合计（优先匹配"价税合计（小写）"上下文，避免误匹配大写金额）
    total_amount_patterns = [
        r'价税合计[（(]小写[）)]\s*[：:￥¥]?\s*([\d,]+\.\d{2})',
        r'价税合计.*?([\d,]+\.\d{2})',
        r'价税合计[（(]大写[）)].*?[\d,]+\.\d{2}',  # 跳过大写，匹配后续小写
        r'合计.*?([\d,]+\.\d{2})',
        r'￥\s*([\d,]+\.\d{2})',
        r'¥\s*([\d,]+\.\d{2})',
    ]
    ...
    # 【价税勾稽校验】不含税金额 + 税额 ≈ 价税合计（允许1.00元以内四舍五入差异）
    # 符合 AGENTS.md 5.5 节"勾稽校验强制化"规则
    amount_excl_tax = data.get("amount_excl_tax")
    tax_amount = data.get("tax_amount")
    total_amount = data.get("total_amount")
    if amount_excl_tax and tax_amount and total_amount:
        calculated_total = amount_excl_tax + tax_amount
        ...
        if difference > Decimal("1.00"):
            ...
            data["validation_warnings"].append(
                f"价税勾稽校验异常：不含税{amount_excl_tax}+税额{tax_amount}"
                f"={calculated_total}，与价税合计{total_amount}差异{difference}元"
            )
        else:
...
def parse_bank_statement_rules(text: str, file_path: str = "") -> dict[str, Any]:
    ...
    # 提取交易记录（支持空格分隔和CSV逗号分隔两种格式）
    lines = text.split('\n')
    transactions = []

    # 检测是否为CSV格式（首行或数据行包含逗号分隔的字段）
    is_csv_format = False
    csv_header_fields = ["交易日期", "对方户名", "交易金额", "摘要", "对方账号", "余额"]
    for line in lines[:5]:
        line_stripped = line.strip()
        if not line_stripped:
            continue
        # 如果行中包含2个以上逗号，且包含交易相关关键词，判定为CSV格式
        comma_count = line_stripped.count(',')
        if comma_count >= 2 and any(kw in line_stripped for kw in csv_header_fields):
            is_csv_format = True
            break

    if is_csv_format:
        # CSV格式解析：支持"交易日期,对方户名,交易金额,摘要"等格式
        # 跳过表头行（非数据行）
        for line in lines:
            line = line.strip()
            if not line:
            ...
            if len(fields) >= 3:
                ...
                desc_fields = []
                for field in fields:
                    date_match = re.search(r'(\d{4}-\d{2}-\d{2})', field)
                    amount_match = re.search(r'-?([\d,]+\.\d{2})', field)
                    if date_field is None and date_match:
                        date_field = date_match.group(1)
                    elif amount_field is None and amount_match:
                        # 保留原始金额（含负号）
                        amount_field = field.strip()
                    else:
                        desc_fields.append(field)
                if date_field and amount_field:
                    transactions.append({
                        "transaction_date": date_field,
                        "description": " ".join(desc_fields) if desc_fields else "",
                        "amount": amount_field.replace(',', ''),
                        "counterparty_name": desc_fields[0] if desc_fields else None,
                        "summary": " ".join(desc_fields) if desc_fields else "",
                    })
    else:
        # 空格分隔格式解析（原有逻辑）
        for line in lines:
    ...
    for trans in transactions:
        ...
        if isinstance(desc, str) and len(desc) > 2:
            if trans.get("counterparty_name") is None:
