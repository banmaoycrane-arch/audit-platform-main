...
from app.services.doc_parsing.parser_engine.auto_archive_service import auto_review_and_archive
...
def convert_parse_result_to_dict(parse_result: object) -> dict[str, Any]:
    ...
    if isinstance(parse_result, LLMComparisonResult):
        ...
        if final:
            fmt = final.file_format
            file_format = fmt.value if fmt and hasattr(fmt, "value") else str(fmt) if fmt else "unknown"
            return {
                "file_format": file_format,
                "document_type": final.document_type.value if hasattr(final.document_type, "value") else str(final.document_type),
                "document_sub_type": (
    ...
    if isinstance(parse_result, ParseResult):
        ...
        return {
            ...
            "seal_info": parse_result.seal_info.to_dict() if parse_result.seal_info else None,
        }
...
def build_parser_engine_summary(
    job: ImportJob,
...
def parse_source_file_with_unified_engine(db: Session, job: ImportJob, source_file: SourceFile) -> tuple[dict[str, Any], dict[str, Any]]:
...
def mark_parser_engine_failure(db: Session, job: ImportJob, source_file: SourceFile, error_message: str) -> None:
    db.rollback()
    job_refreshed = db.get(ImportJob, job.id)
    source_file_refreshed = db.get(SourceFile, source_file.id)
    if not job_refreshed or not source_file_refreshed:
        return
    job = job_refreshed
    source_file = source_file_refreshed
