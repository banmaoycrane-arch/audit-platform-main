...
class EntryLike(Protocol):
    ...
    debit_amount: Decimal | float | None
    credit_amount: Decimal | float | None
    voucher_date: Any
...
VOUCHER_TYPE_PATTERNS: dict[str, dict[str, Any]] = {
    "银": {
        "keywords": [
            # 银行相关科目
...
def suggest_voucher_type(entry: Any) -> tuple[str | None, float]:
    """
    根据分录内容推荐凭证字
    ...
    account = (getattr(entry, "account_name", "") or "").lower()
...
def suggest_tags(entry: EntryLike) -> list[tuple[str, float]]:
    """推荐标签"""
