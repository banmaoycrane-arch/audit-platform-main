...
from app.services.basic_data.register_ingestion_service import classify_and_ingest_register
...
from app.services.audit.risk_rule_service import generate_risks
from app.services.audit.risk_case_library import enhance_entry_with_risk_analysis
from app.services.shared.ledger_context_service import resolve_or_create_organization_for_ledger
...
def _build_voucher_group_lines(
    ...
    existing_keys: set[tuple[str, str, str, str, str, str, str, str]],
    source_file_id: int,
    ...
    for raw_index, entry_data in group_items:
        ...
        line = VoucherEntryLine(
            account_code=str(entry_data.get("account_code") or ""),
            ...
            for entry, semantic_text in zip(created_entries, semantic_texts):
                ...
                _index_text(
                    ...
                    semantic_text,
                    {
                        ...
                        "amount": str(entry.debit_amount or entry.credit_amount or Decimal("0.00")),
...
def _process_source_file(db: Session, job: ImportJob, source_file: SourceFile) -> ProcessingResult:
    ...
    try:
        ...
        # classify_document 期望接收文件路径，由它内部读取并识别
        classification = classify_document(path, source_file.filename)
        feedback = _extract_source_summary(classification)
        feedback.update({
...
def process_import_job(
    ...
    *,
    ...
    if all_logic_results:
        ...
        merged_logic_report = generate_batch_report(merged_results)
    ...
    return ImportReport(
        ...
        register_summary=register_summary or None,
...
def get_import_summary(report: ImportReport) -> dict[str, Any]:
    ...
    day_book_summary = None
    if report.day_book_report is not None:
        day_book_summary = {
            "total_vouchers": report.day_book_report.total_vouchers,
            "total_entries": report.day_book_report.total_entries,
            "skip_count": report.day_book_report.skip_count,
            "unbalanced_count": report.day_book_report.unbalanced_count,
            "completeness_score": report.day_book_report.completeness_score,
            "missing_voucher_nos": report.day_book_report.missing_voucher_nos,
            "unbalanced_vouchers": [
                {
                ...
                for v in report.day_book_report.unbalanced_vouchers
            ],
        }
    return {
