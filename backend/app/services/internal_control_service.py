...
class InternalControlService:
    ...
    def execute_control_test(self, organization_id: int, control_id: int, 
                             ...
                             evidence_missing: List[str] = None) -> ControlTest:
    ...
    def _determine_alert(self, overall_risk: float, missing: List[str]) -> tuple:
    ...
    def _create_control_alert(self, organization_id: int, control_id: int, 
                              ...
                              control_risk: float = 0.8, detection_risk: float = 0.5):
        control = self.get_control(control_id)
        if not control:
            return
