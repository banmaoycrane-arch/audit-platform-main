...
class TransactionManager:
    ...
    def begin_transaction(self, transaction_type: str, context_id: Optional[int] = None, 
                          context_type: Optional[str] = None) -> Any:
        """开始一个事务"""
    ...
    def rollback_transaction(self, transaction_id: int, error_message: str = "") -> None:
        """回滚事务"""
        transaction = self._get_transaction(transaction_id)
        if not transaction:
            return
    ...
    def _rollback_operation(self, operation: TransactionOperation) -> None:
        """回滚单个操作"""
    ...
    def _get_entity_model(self, entity_type: str) -> Optional[Type[Any]]:
        """根据实体类型获取对应的模型类"""
        return ENTITY_MODEL_MAP.get(entity_type)

    def _delete_entity(self, entity_type: str, entity_id: int) -> None:
        """删除实体"""
    ...
    def _restore_entity(self, entity_type: str, entity_id: int, rollback_details: Dict[str, Any]) -> None:
        """恢复实体到之前的状态"""
        model_class = self._get_entity_model(entity_type)
    ...
    def _restore_deleted_entity(self, entity_type: str, rollback_details: Dict[str, Any]) -> None:
        """重新创建删除的实体"""
        model_class = self._get_entity_model(entity_type)
        if model_class and rollback_details:
    ...
    def record_operation(self, transaction_id: int, operation_type: str, 
                         entity_type: str, operation_details: Dict[str, Any],
                         rollback_details: Optional[Dict[str, Any]] = None) -> TransactionOperation:
        """记录操作"""
        transaction = self._get_transaction(transaction_id)
        if not transaction:
    ...
    def mark_operation_success(self, operation_id: int, entity_id: int) -> None:
        """标记操作成功"""
    ...
    def mark_operation_failure(self, operation_id: int, error_message: str) -> None:
        """标记操作失败"""
        operation = self.db.query(TransactionOperation)\
            .filter(TransactionOperation.id == operation_id)\
            .first()
    ...
    def add_checkpoint(self, transaction_id: int, checkpoint_name: str) -> TransactionCheckpoint:
        """添加检查点"""
        transaction = self._get_transaction(transaction_id)
        if not transaction:
            raise ValueError("Transaction not found")
    ...
    def mark_checkpoint_reached(self, checkpoint_id: int) -> None:
        """标记检查点已到达"""
    ...
    def cleanup_stale_transactions(self, hours_threshold: int = 24) -> None:
        """清理长时间未完成的事务"""
