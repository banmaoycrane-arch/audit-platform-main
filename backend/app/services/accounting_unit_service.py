...
class AccountingUnitService:
    ...
    def _create_hierarchy(self, parent_id: int, child_id: int):
        """创建层级关系"""
        parent = self.db.query(AccountingUnit).filter(AccountingUnit.id == parent_id).first()
    ...
    def add_to_combination(self, combination_id: int, unit_id: int, weight: float = 1.0):
        """添加核算单位到组合"""
    ...
    def remove_from_combination(self, combination_id: int, unit_id: int):
        """从组合中移除核算单位"""
    ...
    def link_to_entity(self, unit_id: int, entity_id: int, relation_type: str = "primary"):
        """关联核算单位到会计主体"""
    ...
    def record_version(self, unit_id: int, changes: Dict, change_reason: Optional[str] = None):
        """记录核算单位版本"""
    ...
    def create_version(self, unit_id: int, version_name: str, effective_date,
                       changes: Dict, change_reason: Optional[str] = None,
    ...
    def add_tag(self, unit_id: int, tag: str, tag_type: str = "name", confidence: float = 0.8):
        """添加核算单位标签"""
