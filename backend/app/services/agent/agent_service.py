...
"""
模块功能：AI代理服务
业务场景：处理用户意图识别、任务规划、与LLM交互
政策依据：无
输入数据：用户消息、用户ID、账簿ID、LLM客户端
输出结果：意图识别结果、任务计划、代理响应
创建日期：2025-01-20
更新记录：
    2025-01-20  将函数封装为 AgentService 类
"""
...
TASK_PLAN_RULES: dict[str, dict[str, Any]] = {
    "accounting_import": {
...
class AgentService:
    ...
    功能描述：处理用户意图识别、任务规划和与LLM的交互
    业务逻辑：根据用户消息识别意图，生成任务计划，调用LLM获取响应
    会计口径：无特殊会计口径要求
    
    注意事项：
        1. Agent 只能生成建议、草稿和风险提示
        2. 正式操作需要人工确认
    """
    ...
    def detect_intent(self, message: str) -> dict[str, Any]:
        ...
        Returns:
            dict: 意图识别结果，包含 intent、confidence、reply、suggested_path、steps
        """
        text = message.lower()
    ...
    def plan_task(self, intent: str, message: str) -> dict[str, Any]:
        """
        根据意图规划任务
        ...
        plan = self.detect_intent(message)
    ...
    def normalize_agent_response(
        ...
        Args:
            raw: 原始响应
            fallback: 备用响应
            source: 响应来源 (rules/llm)
            model_available: LLM模型是否可用
    ...
    def plan_agent_task(
        ...
        Returns:
            dict: 包含任务计划的完整结果
        """
        agent_result = self.chat_with_agent(message, llm_client=llm_client)
        agent_result["task_plan"] = self.build_task_plan(agent_result, user_id, ledger_id)
        return agent_result

    def chat_with_agent(self, message: str, llm_client: Any | None = None) -> dict[str, Any]:
        """
        与代理对话
        ...
        Returns:
            dict: 代理响应结果
        """
        fallback = self.detect_intent(message)
        client = llm_client or LightweightLLMClient()

        if not client.is_configured():
            return self.normalize_agent_response(fallback, fallback, "rules", False)
        ...
        return self.normalize_agent_response(raw, fallback, "llm", True)
...
def detect_intent(message: str) -> dict[str, Any]:
    return _agent_service_instance.detect_intent(message)


def plan_task(intent: str, message: str) -> dict[str, Any]:
    return _agent_service_instance.plan_task(intent, message)
...
def normalize_agent_response(
    ...
    return _agent_service_instance.normalize_agent_response(raw, fallback, source, model_available)
...
def build_task_plan(
    ...
    return _agent_service_instance.build_task_plan(agent_result, user_id, ledger_id)
...
def plan_agent_task(
    ...
    return _agent_service_instance.plan_agent_task(message, user_id, ledger_id, llm_client)


def chat_with_agent(message: str, llm_client: Any | None = None) -> dict[str, Any]:
    return _agent_service_instance.chat_with_agent(message, llm_client)
