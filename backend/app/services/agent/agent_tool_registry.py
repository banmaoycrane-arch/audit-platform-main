...
AGENT_TOOL_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "list_accounting_periods": {
        "tool_name": "list_accounting_periods",
        "description": "查询当前账簿的会计期间列表（期间代码、状态）。",
        "allowed_agent_roles": ["accounting_assistant_agent"],
        "intents": ["period_close", "report_export", "accounting_import"],
        "risk_level": "low",
        "approval_required": False,
        "audit_trace_required": True,
    },
    "list_evidence_inbox": {
        "tool_name": "list_evidence_inbox",
        "description": "查询证据云空间收件箱或归档库中的文件清单（只读）。",
        "allowed_agent_roles": ["accounting_assistant_agent", "accounting_clerk_agent"],
        "intents": ["accounting_import", "due_diligence_audit"],
        "risk_level": "low",
        "approval_required": False,
    ...
    "list_import_jobs": {
        "tool_name": "list_import_jobs",
        "description": "查询当前账簿的资料导入任务列表。",
        "allowed_agent_roles": ["accounting_assistant_agent", "accounting_clerk_agent"],
        "intents": ["accounting_import", "audit_workflow"],
        "risk_level": "low",
        "approval_required": False,
        "audit_trace_required": True,
    },
    "get_trial_balance": {
        ...
        "description": "生成指定期间的科目余额表摘要（只读）。",
        "allowed_agent_roles": ["accounting_assistant_agent", "report_agent"],
        "intents": ["period_close", "report_export"],
        "risk_level": "low",
        "approval_required": False,
        "audit_trace_required": True,
    },
    "list_internal_control_findings": {
        "tool_name": "list_internal_control_findings",
        "description": "查询内控待办/缺陷提醒列表（只读）。",
        "allowed_agent_roles": ["audit_assistant_agent", "accounting_assistant_agent"],
        "intents": ["audit_workflow", "due_diligence_audit"],
        "risk_level": "low",
        "approval_required": False,
        "audit_trace_required": True,
...
def list_allowed_tools_for_intent(intent: str, agent_role: str | None = None) -> list[dict[str, Any]]:
    """
...
def get_agent_tool(tool_name: str) -> dict[str, Any] | None:
    """
    功能描述：根据工具名称获取白名单配置。
