...
更新记录：
    ...
    2025-01-20  封装为 AgentApprovalService 类
"""
...
class AgentApprovalService:
    ...
    功能描述：为中高风险 Agent 工具生成待确认记录，并记录人工确认结果
    ...
    def serialize_agent_approval(self, approval: AgentApproval) -> dict[str, Any]:
        """
        把 Agent 确认记录转换为接口返回结构
...
_approval_service_instance = AgentApprovalService()
...
def request_agent_tool_approval(
    ...
    return _approval_service_instance.request_agent_tool_approval(
        db, tool_name, agent_role, args, current_user, ledger_id
    )
...
def confirm_agent_tool_approval(
    ...
    return _approval_service_instance.confirm_agent_tool_approval(
        db, approval_id, current_user, comment
    )


def serialize_agent_approval(approval: AgentApproval) -> dict[str, Any]:
    return _approval_service_instance.serialize_agent_approval(approval)
