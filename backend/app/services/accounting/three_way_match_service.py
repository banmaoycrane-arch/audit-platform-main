...
def _amounts_close(left: float | Decimal | str | int, right: float | Decimal | str | int) -> bool:
    return abs(_to_decimal(left) - _to_decimal(right)) <= AMOUNT_TOLERANCE
...
def _sum_amounts(items: list[Any], attr: str = "total_amount") -> Decimal:
    return sum((_to_decimal(getattr(item, attr, 0)) for item in items), Decimal("0.00"))
        ...
        checks.append(
            {
                ...
                "label": "合同金额 vs 发票合计",
                "left_amount": float(contract_amount),
                "right_amount": float(invoice_total),
        ...
        checks.append(
            {
                ...
                "label": "合同金额 vs 入库合计",
                "left_amount": float(contract_amount),
                "right_amount": float(inventory_total),
        ...
        checks.append(
            {
                ...
                "label": "发票合计 vs 入库合计",
                "left_amount": float(invoice_total),
                "right_amount": float(inventory_total),
...
def match_purchase_contract(db: Session, ledger_id: int, contract_id: int) -> dict[str, Any]:
    ...
    invoice_total = sum((_to_decimal(item.total_amount) for item in invoices), Decimal("0.00"))
    inventory_total = sum((_to_decimal(item.total_amount) for item in inventory_docs), Decimal("0.00"))
