...
from app.db.models import (
    ...
    Company, CompanyPersonnel, RelatedPartyRelation, FieldAliasMapping, DocumentTag,
    Organization
)
...
class DocumentParsingService:
    ...
    def parse_contract(self, organization_id: int, contract_data: Dict[str, Any]) -> Contract:
        ...
        contract = Contract(
            ...
            tax_amount=contract_data.get("tax_amount"),
            performance_obligations=contract_data.get("performance_obligations", {}),
            transaction_price=contract_data.get("transaction_price"),
            standalone_price=contract_data.get("standalone_price"),
    ...
    def _add_contract_party(self, contract_id: int, party_data: Dict[str, Any]):
        party = ContractParty(
    ...
    def _add_performance_obligation(self, contract_id: int, obligation_data: Dict[str, Any]):
        obligation = ContractPerformanceObligation(
            ...
            standalone_price=obligation_data.get("standalone_price"),
            allocated_price=obligation_data.get("allocated_price"),
            ...
            revenue_recognized=obligation_data.get("revenue_recognized"),
            revenue_pending=obligation_data.get("revenue_pending")
        )
        self.db.add(obligation)

    def _add_payment_term(self, contract_id: int, term_data: Dict[str, Any]):
        term = ContractPaymentTerm(
            ...
            actual_paid=term_data.get("actual_paid"),
            paid_date=term_data.get("paid_date")
        )
        self.db.add(term)

    def _generate_contract_tags(self, contract_id: int, contract_data: Dict[str, Any]):
        tags = []
        
        tags.append({"tag": contract_data.get("contract_type", "service"), "tag_type": "business"})
        
        if contract_data.get("is_over_time"):
            tags.append({"tag": "over_time_revenue", "tag_type": "risk"})
        
        amount = contract_data.get("contract_amount", 0)
        if amount and amount > 1000000:
            tags.append({"tag": "large_amount", "tag_type": "amount"})
        
        if contract_data.get("end_date"):
            tags.append({"tag": f"expire_{contract_data['end_date'].year}", "tag_type": "time"})
    ...
    def _add_invoice_item(self, invoice_id: int, item_data: Dict[str, Any]):
        item = InvoiceItem(
    ...
    def parse_inventory_document(self, organization_id: int, doc_data: Dict[str, Any]) -> InventoryDocument:
        document = InventoryDocument(
            ...
            total_quantity=doc_data.get("total_quantity"),
    ...
    def _add_inventory_item(self, document_id: int, item_data: Dict[str, Any]):
        item = InventoryItem(
    ...
    def parse_bank_statement(self, organization_id: int, statement_data: Dict[str, Any]) -> BankStatement:
        statement = BankStatement(
            ...
            amount=statement_data.get("amount"),
    ...
    def initialize_default_aliases(self):
        aliases = [
