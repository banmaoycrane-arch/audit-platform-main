...
from app.services.shared.ledger_service import (
    LedgerService,
    LedgerEntry,
    ContractLedger,
    InvoiceLedger,
...
class AuditTestService:
    ...
    def match_entries(
        ...
        amount_ok = bool(match_details.get("amount_match"))
        date_ok = bool(match_details.get("date_match"))
        matched = bool(match_score >= 0.7 or (amount_ok and date_ok))
    ...
    def build_business_chain(
        ...
        # 获取所有台账
        ledgers_by_type: Dict[str, Dict[str, Any]] = {
    ...
    def classification_test(
        self,
        business_type: BusinessType,
        voucher_type_mapping: Dict[str, Any] | None = None
    ) -> Dict[str, Any]:
        """
        分类测试：检查凭证字与业务类型是否匹配
    ...
    def reset_ledger_status(self) -> None:
        """重置台账条目的匹配状态"""
        ledgers: list[Dict[str, Any]] = [
            self.ledger_service.contract_ledger,
            self.ledger_service.invoice_ledger,
