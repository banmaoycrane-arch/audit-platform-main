...
def _get_notification_context(db: Session, target_type: str, target_id: int) -> tuple[int | None, int | None]:
    if target_type == "task":
        task = db.get(AuditTask, target_id)
        return (task.project_id, task.ledger_id) if task else (None, None)
    if target_type == "branch":
        branch = db.get(AuditWorkBranch, target_id)
        return (branch.project_id, branch.ledger_id) if branch else (None, None)
    if target_type == "review_request":
        review = db.get(AuditReviewRequest, target_id)
        return (review.project_id, review.ledger_id) if review else (None, None)
