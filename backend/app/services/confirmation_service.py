...
from app.services.module_register_service import BALANCE_TYPE_LABELS, list_counterparty_balances
...
def create_from_balances(
    ...
    *,
    ...
    for bucket in balances:
        ...
        book_balance = _round_amount(bucket["total_amount"]) or 0.0
        if book_balance <= 0:
            continue
...
def _apply_reply_difference(row: CounterpartyConfirmation, reply_amount: float) -> None:
    row.reply_amount = _round_amount(reply_amount)
    confirmation_amount = _round_amount(row.confirmation_amount) or 0.0
    difference = _round_amount((row.reply_amount or 0) - confirmation_amount)
    row.difference = difference
    row.replied_at = datetime.utcnow()
    row.status = "exception" if difference is not None and abs(difference) >= 0.01 else "replied"
...
def update_confirmation(
    ...
    *,
    ...
    confirmation_amount: float | None = None,
    ...
    if confirmation_amount is not None:
        row.confirmation_amount = _round_amount(confirmation_amount) or 0.0
