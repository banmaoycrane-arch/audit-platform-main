...
@router.post("")
def create_counterparty(payload: CounterpartyCreate, db: Session = Depends(get_db)) -> dict:
...
@router.put("/{cp_id}")
def update_counterparty(cp_id: int, payload: CounterpartyUpdate, db: Session = Depends(get_db)) -> dict:
...
def disable_counterparty(cp_id: int, db: Session = Depends(get_db)) -> dict:
    cp = db.get(Counterparty, cp_id)
    if not cp:
        raise HTTPException(status_code=404, detail="对方单位不存在")
    cp.is_active = False
